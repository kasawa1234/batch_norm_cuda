...
template <typename scalar_t>
__global__ void mean_kernel_conv(
...
template <typename scalar_t>
__global__ void std_kernel_conv(
    torch::PackedTensorAccessor32<scalar_t, 4, torch::RestrictPtrTraits> input_data,
    torch::PackedTensorAccessor32<scalar_t, 1, torch::RestrictPtrTraits> mean,
...
torch::Tensor bn_forward_conv_cuda(
    ...
    // launch the kernel
    AT_DISPATCH_FLOATING_TYPES(X.type(), "mean_kernel_conv",
    ([&] {
        mean_kernel_conv<scalar_t><<<blocks_mean, threads_mean>>>(
    ...
    // launch the kernel
    AT_DISPATCH_FLOATING_TYPES(X.type(), "std_kernel_conv",
    ([&] {
        std_kernel_conv<scalar_t><<<blocks_mean, threads_mean>>>(
            X.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(),
            mean.packed_accessor32<scalar_t, 1, torch::RestrictPtrTraits>(),
